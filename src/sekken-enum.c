 ...
 *
 * An Active Directory enumeration tool that leverages the ADWS protocol (TCP 9389)
 */
...
DECLSPEC_IMPORT SECURITY_STATUS WINAPI SECUR32$InitializeSecurityContextW(PCredHandle, PCtxtHandle, LPWSTR, ULONG,
...
VOID go(IN PCHAR Buffer, IN ULONG Length)
{
    ...
    char *attrFilter = NULL;
    char *customBaseDN = NULL;
    ...
    if (Length == 0)
    {
        BeaconPrintf(CALLBACK_ERROR, "[-] Usage: sekken-enum [target] <ldap_filter> [attributes] [-b custom_base_dn]");
        BeaconPrintf(CALLBACK_ERROR, "[-] Example: sekken-enum \"(samaccountname=admin*)\"");
        BeaconPrintf(CALLBACK_ERROR, "[-] Example: sekken-enum dc.domain.local \"(samaccountname=admin*)\"");
        BeaconPrintf(CALLBACK_ERROR,
                     "[-] Example: sekken-enum dc.domain.local \"(objectclass=user)\" \"samaccountname,memberof\"");
        BeaconPrintf(CALLBACK_ERROR,
                     "[-] Example: sekken-enum dc.domain.local \"(objectclass=pKICertificateTemplate)\" "
                     "\"cn,displayname\" -b \"CN=Configuration,DC=domain,DC=local\"");
        BeaconPrintf(CALLBACK_ERROR, "[-] Note: If target is omitted, DC will be auto-discovered");
        return;
    }
    else
    {
        ...
        if (arg1 && arg1[0] == '"' && arg1[MSVCRT$strlen(arg1) - 1] == '"')
        {
            arg1[MSVCRT$strlen(arg1) - 1] = '\0';
            arg1++;
        }
        if (arg2 && arg2[0] == '"' && arg2[MSVCRT$strlen(arg2) - 1] == '"')
        {
            arg2[MSVCRT$strlen(arg2) - 1] = '\0';
            arg2++;
        }
        if (arg3 && arg3[0] == '"' && arg3[MSVCRT$strlen(arg3) - 1] == '"')
        {
            arg3[MSVCRT$strlen(arg3) - 1] = '\0';
            arg3++;
        }
        ...
        if (parser.length > 0)
        {
            ...
            if (flag && MSVCRT$strcmp(flag, "-b") == 0)
            {
                tmpBaseDN = BeaconDataExtract(&parser, NULL);
            }
        }

        // Allocate on heap
        target = (char *)intAlloc(256);
        ldapFilter = (char *)intAlloc(256);
        attrFilter = (char *)intAlloc(256);
        if (tmpBaseDN)
        {
            customBaseDN = (char *)intAlloc(512);
        }

        if (!target || !ldapFilter || !attrFilter || (tmpBaseDN && !customBaseDN))
        {
            BeaconPrintf(CALLBACK_ERROR, "[-] Memory allocation failed");
            return;
        }

        BOOL isArg1Filter = (arg1 && MSVCRT$strlen(arg1) > 0 && arg1[0] == '(');
        BOOL isArg1Empty = (!arg1 || MSVCRT$strlen(arg1) == 0);

        if (isArg1Filter || isArg1Empty)
        {
            // arg1 is filter or empty, so auto-discover DC
            if (!GetDomainController(target, 256))
            {
                ...
                intFree(attrFilter);
                if (customBaseDN)
                    intFree(customBaseDN);
                return;
            }

            if (isArg1Filter)
            {
                // arg1 = filter, arg2 = attributes
                MSVCRT$strncpy(ldapFilter, arg1, 255);
                ldapFilter[255] = '\0';

                if (arg2 && MSVCRT$strlen(arg2) > 0 && arg2[0] != '(')
                {
                    MSVCRT$strncpy(attrFilter, arg2, 255);
                    attrFilter[255] = '\0';
                }
                else
                {
            ...
            {
                // arg1 is empty, arg2 might be filter
                if (arg2 && MSVCRT$strlen(arg2) > 0 && arg2[0] == '(')
                {
                    MSVCRT$strncpy(ldapFilter, arg2, 255);
                    ldapFilter[255] = '\0';
                }
                else
                {
                    MSVCRT$strcpy(ldapFilter, "(objectclass=*)");
                }

                // arg3 is attributes only if it doesn't look like a filter
                if (arg3 && MSVCRT$strlen(arg3) > 0 && arg3[0] != '(')
                {
                    MSVCRT$strncpy(attrFilter, arg3, 255);
                    attrFilter[255] = '\0';
                }
                else
                {
        ...
        {
            MSVCRT$strncpy(target, arg1, 255);
            target[255] = '\0';

            if (arg2 && MSVCRT$strlen(arg2) > 0)
            {
                MSVCRT$strncpy(ldapFilter, arg2, 255);
                ldapFilter[255] = '\0';
            }
        ...
        if (tmpBaseDN && MSVCRT$strlen(tmpBaseDN) > 0)
        {
            MSVCRT$strncpy(customBaseDN, tmpBaseDN, 511);
            customBaseDN[511] = '\0';
        }
    }
    ...
    BeaconPrintf(CALLBACK_OUTPUT, "[*] Attributes: %s\n", attrFilter ? attrFilter : "ALL");

    CONNECTION_CONTEXT ctx = {0};
    int ret;
    char *baseDN = (char *)intAlloc(512);
    if (!baseDN)
    {
        BeaconPrintf(CALLBACK_ERROR, "[-] Failed to allocate memory for baseDN");
        goto cleanup_params;
    }

    BeaconPrintf(CALLBACK_OUTPUT, "[*] Connecting to ADWS on %s...\n", target);

    if (customBaseDN)
    {
        MSVCRT$strncpy(baseDN, customBaseDN, 511);
        baseDN[511] = '\0';
    }
    else
    {
    ...
    ret = WS2_32$WSAStartup(MAKEWORD(2, 2), &ctx.wsaData);
    if (ret != 0)
    {
        BeaconPrintf(CALLBACK_OUTPUT, "[-] WSAStartup failed: %d\n", ret);
        return;
    }
    ...
    BeaconPrintf(CALLBACK_OUTPUT, "[+] Connected to ADWS\n");
    ...
    if (!nbfseBuffer)
    {
        BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to create NBFSE buffer");
        goto cleanup_params;
    }

    if (!BuildEnumerateRequest(nbfseBuffer, ldapFilter, baseDN, target, attrFilter))
    {
        BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to build enumerate request");
    ...
    BeaconPrintf(CALLBACK_OUTPUT, "[*] Retrieving objects...");
